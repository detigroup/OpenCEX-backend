...
SUMSUB_SECRET_KEY = "hquEE4bWELDdt16e9tU6AzJipQhXbmrG"  # Example: Hej2ch71kG2kTd1iIUDZFNsO5C1lh5Gq
SUMSUB_APP_TOKEN = "sbx:F3P4J2GnDYHTsGiHivJv7hxj.uJEuxWbNxMpgyyHj4BurrKOsbLHpcrGW"  # Example: sbx:uY0CgwELmgUAEyl4hNWxLngb.0WSeQeiYny4WEqmAALEAiK2qTC96fBad
...
def sign_request(request: requests.Request) -> requests.PreparedRequest:
    ...
    # logging.info("data_to_sign: %s", data_to_sign)
    # hmac needs bytes
    signature = hmac.new(
    ...
    # logging.info("headers: %s", prepared_request.headers)
...
def main(external_user_id = "baysao@gmail.com"):
    # logging.basicConfig(level=logging.INFO)
    # external_user_id = 'baysao@gmail.com' #str(uuid.uuid4())
    level_name = 'id-only'
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        # The first argument after the script name is sys.argv[1]
        name = sys.argv[1]
        # logging.info(f"Running main with argument: {name}")
        # 
        exit(main(name))
    else:
